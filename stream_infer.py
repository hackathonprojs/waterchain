...
NETWORK_CATEGORIES_TXT = "WaterNet/labels.txt" # categories.txt for network
...
CAMERA_INDEX = "1"              # 0 is first usb cam, 1 the second etc.
...
def postprocess(output):
    ...
    if detection != gNetworkCategories[order[last-i]]:
        ...
        #datastring=str(json.dumps({'confidence':output[order[last-0]]*100, 'type':gNetworkCategories[order[last-i]]}))
        datastring=datastring='confidence={0:.2f}'.format(output[order[last-0]]*100) + '&type=' + gNetworkCategories[order[last-i]]
        print(datastring)
